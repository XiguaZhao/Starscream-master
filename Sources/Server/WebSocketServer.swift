...
public class ServerConnection: Connection, HTTPServerDelegate, FramerEventClient, FrameCollectorDelegate, TransportEventClient {
    ...
    public weak var groupDelegate: ConnectionDelegate?
    ...
    public func didReceive(event: HTTPEvent) {
        ...
        case .success(let headers):
            ...
            groupDelegate?.didReceive(event: .connected(self, headers))
    ...
    public func didForm(event: FrameCollector.Event) {
        ...
        case .text(let string):
            groupDelegate?.didReceive(event: .text(self, string))
            onEvent?(.text(string))
        case .binary(let data):
            groupDelegate?.didReceive(event: .binary(self, data))
            onEvent?(.binary(data))
        case .pong(let data):
            groupDelegate?.didReceive(event: .pong(self, data))
            onEvent?(.pong(data))
        case .ping(let data):
            groupDelegate?.didReceive(event: .ping(self, data))
            onEvent?(.ping(data))
        case .closed(let reason, let code):
            groupDelegate?.didReceive(event: .disconnected(self, reason, code))
